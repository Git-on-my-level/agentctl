...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"run": {
			Name: "run", Summary: "Launch an exact native argv and persist its normalized execution and final result.", Usage: "agentctl run [--adapter name] [--timeout duration|--no-timeout] [flags] -- <native argv>",
			SideEffectClass: output.ExternalSideEffect, Defaults: []string{"adapter inferred from known executable names", "30 minute timeout", "launch and result_content preflight", "bounded final result storage"},
		...
		"config":       {Name: "config", Summary: "Manage executable provenance expectations, optional authority, and explicit local configuration bundles.", Usage: "agentctl config set-profile|show|validate|doctor|bundle ...", SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"no implicit unauthenticated Multica profile", "private bundles require explicit --config-bundle and apply additively", "native run argv remains caller-authoritative"}, Examples: [][]string{{"agentctl", "config", "show"}, {"agentctl", "--config-bundle", "/local/config-bundle.json", "config", "bundle", "plan"}}, Related: []string{"doctor", "promote"}},
...
func (a *app) help(renderer output.Renderer, rawTopic string) int {
	...
	if topic != "" {
		aliases := map[string]string{
			...
			"config set-profile": "config", "config show": "config", "config validate": "config", "config doctor": "config", "config bundle": "config", "config bundle validate": "config", "config bundle show": "config", "config bundle plan": "config",
			"data inventory":     "data",
