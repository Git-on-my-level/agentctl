...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"route":         {Name: "route", Summary: "Explain a ranked host/model match from a selector string, or the legacy flag-based lifecycle recommendation.", Usage: "agentctl route explain [flags] [query...]", SideEffectClass: output.ReadOnly, Defaults: []string{"a query string ranks hosts and models; flags without a query keep the legacy direct-versus-authority recommendation"}, Examples: [][]string{{"agentctl", "route", "explain", "--", "studio", "omp"}, {"agentctl", "route", "explain", "--model-family", "gpt"}}, Related: []string{"run", "promote"}},
