...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"delegate": {
			Name: "delegate", Summary: "Resolve structured model constraints, launch native work once, and optionally collect its successful answer.",
			Usage:           "agentctl delegate --request-file path (--prompt-file path|--prompt-stdin) [--cwd path] [--plan] [--wait [--content] [--require-result-source source] [--min-result-bytes n]] [--timeout duration] [--label name ...] [--authority auto|native|multica]",
			SideEffectClass: output.ExternalSideEffect,
			Defaults:        []string{`request example: {"schema_version":1,"request_key":"review-01","selector":{"harness":"cursor","family":"grok","version":"4.6"}}`, "parent extracts only explicit constraints; omitted values use compatible reviewed defaults", "preferred entries are the allowlist; configure family/version metadata; omitted host means local", "conflicting or ambiguous selectors launch nothing; no fuzzy matching or fallback", "request files are regular non-symlinks, strict JSON, at most 64 KiB", "prompt is separate and never journaled; prompt files must be within cwd and at most 8 MiB", "plans may probe versions but never write state or run maintenance", "same profile, request key, and inputs recover original work despite changed defaults; changed inputs conflict", "foreground ownership; no background or external context handles", "wait requires successful completion and nonempty content before collection acknowledgement", "without wait a receipt does not assert successful work", "no default execution timeout", "Cursor trust requires delegation.cursor_workspace_trust", "native settings describe requested identity, not provider attestation", "remote and Multica delegation fail before launch until result and settings guarantees exist"},
			Examples:        [][]string{{"agentctl", "delegate", "--request-file", "request.json", "--prompt-file", "task.md", "--plan"}, {"agentctl", "delegate", "--request-file", "request.json", "--prompt-file", "task.md", "--wait"}},
			Related:         []string{"schema", "config", "run", "dispatch", "result", "recent"},
		},
		"run": {
			Name: "run", Summary: "Launch an exact native argv and persist its normalized execution and terminal result.", Usage: "agentctl run [--execution-id exec-...] [--label name ...] [--task-contract json-file] [--adapter name] [--timeout duration|--no-timeout] [--prompt-file path|--prompt-stdin] [--prompt-delivery argv|stdin] [flags] -- <native argv>",
			SideEffectClass: output.ExternalSideEffect, Defaults: []string{"adapter inferred from known executable names", "execution ID generated unless preallocated", "foreground ownership; --background is rejected", "parent-background a foreground run, or use dispatch for work that must outlive this process", "up to 16 exact metadata labels", "task contracts are bounded strict JSON metadata retained by status and result", "native completion never implies external acceptance", "Multica issues remain authoritative for Multica task contracts", "no wall-clock timeout", "prompt delivery argv when a prompt source is selected", "prompt files are regular non-symlinks within --cwd and at most 8 MiB", "invocation-scoped launch and result_content preflight", "bounded final result storage", "Cursor plan mode rejected because its one-shot completion is unreliable", "off-policy argv against agent_preferences warns and still launches"},
			Examples: [][]string{{"agentctl", "run", "--", "codex", "exec", "--json", "review this change"}, {"agentctl", "run", "--task-contract", "task-contract.json", "--", "codex", "exec", "--json", "perform the bounded task"}, {"agentctl", "run", "--label", "review", "--prompt-file", "task.md", "--prompt-delivery", "argv", "--", "cursor-agent", "--print", "--output-format", "stream-json", "--trust"}, {"agentctl", "run", "--prompt-stdin", "--prompt-delivery", "stdin", "--", "codex", "exec", "--json", "-"}}, Related: []string{"recent", "fanout", "capabilities", "await", "result", "dispatch"},
		},
		"dispatch": {
			...
			SideEffectClass: output.RemoteCoordinationWrite, Defaults: []string{"live agent and runtime resolution from the configured Multica workspace", "exact routed host, adapter, and concrete model required", "off-policy adapter+model against agent_preferences fails closed", "unarchived idle or working agent and online runtime required", "exact assignee and runtime reserved in a local starting execution before the remote call", "replay recovers the prepared target without mutable fleet resolution", "assigned backlog creation is bound locally before todo activation unless --status is explicit", "status must be one of Multica's seven lifecycle statuses", "caller idempotency key required", "prompt delivered only on Multica stdin and never journaled", "returns a Multica-authority exec ID", "no local fallback", "--plan performs live read-only resolution without creating an issue or journal record"},
		...
		"recent":           {Name: "recent", Summary: "Discover recent host-local executions without reading prompt or result records.", Usage: "agentctl recent [--limit n] [--state state|terminal|nonterminal] [--liveness value] [--unreconciled] [--adapter name] [--label name ...]", SideEffectClass: output.ReadOnly, Defaults: []string{"newest 20 executions", "total reports the full filtered set while count reports the returned projection", "host-local journal only", "state and liveness are independent exact filters", "repeated exact label filters use AND semantics", "--unreconciled is terminal work whose result has never been acknowledged", "terminals that predate acknowledgement tracking are not unreconciled", "paths, prompts, transcripts, and results omitted"}, Examples: [][]string{{"agentctl", "recent"}, {"agentctl", "recent", "--state", "nonterminal", "--liveness", "alive", "--label", "review"}, {"agentctl", "recent", "--liveness", "unreachable"}, {"agentctl", "recent", "--unreconciled"}}, Related: []string{"status", "await", "result"}},
		"inbox":            {Name: "inbox", Summary: "Explain host-local executions that need collection, intervention, integrity reconciliation, or bounded stale-work review.", Usage: "agentctl inbox [--limit n] [--stale-after duration] [--adapter name] [--label name ...]", SideEffectClass: output.ReadOnly, Defaults: []string{"newest 20 actionable executions", "total reports the full actionable set while count reports the returned projection", "one hour stale-observation bound", "host-local journal only", "terminal failures leave the inbox when result or terminal await acknowledges collection", "conflicted evidence remains actionable after collection", "work health and tool liveness are reported separately", "no adapter probe, native status refresh, result read, or acknowledgement write", "paths, prompts, transcripts, and results omitted"}, Examples: [][]string{{"agentctl", "inbox"}, {"agentctl", "inbox", "--stale-after", "2h", "--label", "review"}}, Related: []string{"recent", "status", "await", "result"}},
		"workspace":        {Name: "workspace", Summary: "Group the Git-worktree scoped views of host-local executions.", Usage: "agentctl workspace owners [--path directory]", SideEffectClass: output.ReadOnly, Defaults: []string{"owners is the only subcommand; the bare group name is not a command"}, Examples: [][]string{{"agentctl", "workspace", "owners"}}, Related: []string{"workspace owners", "recent", "status"}},
		"workspace owners": {Name: "workspace owners", Summary: "Report nonterminal executions launched from Git worktrees.", Usage: "agentctl workspace owners [--path directory]", SideEffectClass: output.ReadOnly, Defaults: []string{"host-local journal only", "all attributed nonterminal executions when --path is omitted", "exact Git worktree match when --path is supplied", "launch-time provenance, never an exclusive lock", "older nonterminal rows without workspace metadata make evidence_complete false"}, Examples: [][]string{{"agentctl", "workspace", "owners"}, {"agentctl", "workspace", "owners", "--path", "/absolute/worktree"}}, Related: []string{"recent", "status", "schema"}},
		"fanout": {
			Name: "fanout", Summary: "Delegate shared or distinct tasks through explicit native argv vectors with bounded concurrency.", Usage: "agentctl fanout --manifest path [--concurrency n] [--fail-fast] [--plan]",
			SideEffectClass: output.ExternalSideEffect, Defaults: []string{"manifest v1 requires schema_version, children[].argv, and a shared or per-child prompt_file", "prompt files and relative children[].cwd resolve from the manifest directory", "a child without cwd inherits the invoking process directory", "prompt_delivery is argv or stdin globally or per child", "optional unique child names correlate results; shared and child labels are a stable union of at most 16 labels", "all children preflight before any task launch; native read-only version probes may execute", "preflight is not an atomic launch reservation", "foreground same-process coordination", "concurrency 2; callbacks admitted in manifest order", "each unique prompt file read once; 64 MiB total prompt bound", "existing execution IDs fail preflight; no replay or automatic retry", "fail-fast cancels admitted siblings and skips queued children", "per-child launch_attempted, recorded, state, error, and prompt digest; no raw prompts", "no automatic synthesis, group authority, or detached ownership"},
		...
		"result": {
			Name: "result", Summary: "Dereference and optionally assert guarantees on a bounded terminal result, one execution or the whole uncollected set.", Usage: "agentctl result <execution-id> [--content|--summary] [--allow-empty] [--require-result-source source] [--min-result-bytes n] | agentctl result --unreconciled [--label name ...] [--limit n] [--summary] [--allow-empty]",
			SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"stored content or a structured failure is required", "--content writes exact stored UTF-8 text with no envelope and no added newline", "integrity conflicts fail closed", "a successful dereference acknowledges the execution so it is no longer unreconciled", "--unreconciled collects the newest 50 uncollected terminal results through the same single-execution contract", "--unreconciled reports total alongside count so the backlog is countable without paging", "an execution that cannot be dereferenced is reported and skipped instead of failing the batch", "acknowledgement stamps are first-write-wins, so re-running collects only what remains", "--unreconciled rejects --content, --require-result-source, and --min-result-bytes because each describes exactly one result"},
			Examples: [][]string{{"agentctl", "result", "exec-..."}, {"agentctl", "result", "exec-...", "--content"}, {"agentctl", "result", "exec-...", "--require-result-source", "assistant", "--min-result-bytes", "200"}, {"agentctl", "result", "--unreconciled"}, {"agentctl", "result", "--unreconciled", "--label", "review", "--limit", "100", "--summary"}}, Related: []string{"run", "await", "events", "recent", "inbox"},
		},
		"subscribe": {
		...
		"data":          {Name: "data", Summary: "Inventory local journal data and explicitly plan retention cleanup.", Usage: "agentctl data inventory | agentctl data cleanup --before timestamp [--include-unreconciled] --plan | --apply --plan-digest digest", SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"inventory and cleanup planning are read-only", "no automatic retention", "cleanup apply requires the reviewed plan digest", "terminal results nobody collected are protected from cleanup"}, Examples: [][]string{{"agentctl", "data", "inventory"}, {"agentctl", "data", "cleanup", "--before", "2026-07-01T00:00:00Z", "--plan"}}, Related: []string{"data cleanup", "result"}},
		"data cleanup":  {Name: "data cleanup", Summary: "Plan or atomically remove unreferenced terminal execution graphs before a cutoff.", Usage: "agentctl data cleanup --before RFC3339 [--include-unreconciled] --plan | agentctl data cleanup --before RFC3339 [--include-unreconciled] --apply --plan-digest sha256:...", SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"no deletion without --apply and a matching reviewed plan digest", "nonterminal and referenced graphs are protected", "terminal executions whose result was never collected are protected with reason result_unreconciled", "terminals that predate acknowledgement tracking on that journal stay eligible", "--include-unreconciled is the explicit opt-out and changes the plan digest", "protected_unreconciled counts the results retained only because nobody collected them", "physical journal compaction is not automatic"}, Examples: [][]string{{"agentctl", "data", "cleanup", "--before", "2026-07-01T00:00:00Z", "--plan"}, {"agentctl", "data", "cleanup", "--before", "2026-07-01T00:00:00Z", "--apply", "--plan-digest", "sha256:..."}, {"agentctl", "data", "cleanup", "--before", "2026-07-01T00:00:00Z", "--include-unreconciled", "--plan"}}, Related: []string{"data", "result"}},
		...
		"cancel":        {Name: "cancel", Summary: "Request cancellation for native execs or thin-forward Multica-authority execs to issue cancel-task.", Usage: "agentctl cancel <execution-id>", SideEffectClass: output.ExternalSideEffect, Defaults: []string{"native/direct execs use the adapter cancel path and terminalize locally", "Multica-authority execs forward to issue cancel-task using bound issue+task", "Multica success means the cancel request was accepted, not that the remote run has already stopped", "never guesses from a PID"}, Related: []string{"status", "capabilities", "await"}},
