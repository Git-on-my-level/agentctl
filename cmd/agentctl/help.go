...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"config":     {Name: "config", Summary: "Manage exact adapter and Multica authority configuration.", Usage: "agentctl config set-profile|show|validate|doctor ...", SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"no implicit unauthenticated Multica profile"}, Examples: [][]string{{"agentctl", "config", "show"}}, Related: []string{"doctor", "promote"}},
...
func (a *app) help(renderer output.Renderer, rawTopic string) int {
	...
	result := map[string]any{"name": "agentctl", "version": version, "commands": commands, "global_flags": []string{"--output json|text (default json)", "--profile <name>", "--context-file <path>", "--explain", "--config <path>", "--journal <path>"}, "discovery": []string{"agentctl help <topic>", "agentctl doctor"}}
