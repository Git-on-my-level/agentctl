...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"recent": {Name: "recent", Summary: "Discover recent host-local executions without reading prompt or result records.", Usage: "agentctl recent [--limit n] [--state state|terminal|nonterminal] [--adapter name] [--label name ...]", SideEffectClass: output.ReadOnly, Defaults: []string{"newest 20 executions", "host-local journal only", "repeated exact label filters use AND semantics", "paths, prompts, transcripts, and results omitted"}, Examples: [][]string{{"agentctl", "recent"}, {"agentctl", "recent", "--state", "nonterminal", "--label", "review"}}, Related: []string{"status", "await", "result"}},
		"fanout": {
