...
func helpTopics() map[string]commandHelp {
	return map[string]commandHelp{
		...
		"run": {
			Name: "run", Summary: "Launch an exact native argv and persist its normalized execution and terminal result.", Usage: "agentctl run [--execution-id exec-...] [--label name ...] [--background] [--adapter name] [--timeout duration|--no-timeout] [--prompt-file path|--prompt-stdin] [--prompt-delivery argv|stdin] [flags] -- <native argv>",
			SideEffectClass: output.ExternalSideEffect, Defaults: []string{"adapter inferred from known executable names", "execution ID generated unless preallocated", "foreground ownership unless --background is explicit", "background launches use a detached noninteractive host-local worker and return after the execution is journaled", "background workers survive the caller but not a host restart", "direct background execution has no cross-process cancel path unless the adapter advertises one", "background prompt stdin is materialized through a one-shot parent-to-worker pipe", "background cannot use idempotency keys", "up to 16 exact metadata labels", "no wall-clock timeout", "prompt delivery argv when a prompt source is selected", "prompt files are regular non-symlinks within --cwd and at most 8 MiB", "invocation-scoped launch and result_content preflight", "bounded final result storage", "Cursor plan mode rejected because its one-shot completion is unreliable"},
			Examples: [][]string{{"agentctl", "run", "--", "codex", "exec", "--json", "review this change"}, {"agentctl", "run", "--background", "--label", "review", "--prompt-file", "task.md", "--prompt-delivery", "argv", "--", "cursor-agent", "--print", "--output-format", "stream-json", "--trust"}, {"agentctl", "run", "--prompt-stdin", "--prompt-delivery", "stdin", "--", "codex", "exec", "--json", "-"}}, Related: []string{"recent", "fanout", "capabilities", "await", "result", "subscribe"},
		},
		"recent": {Name: "recent", Summary: "Discover recent host-local executions without reading prompt or result records.", Usage: "agentctl recent [--limit n] [--state state|terminal|nonterminal] [--unreconciled] [--adapter name] [--label name ...]", SideEffectClass: output.ReadOnly, Defaults: []string{"newest 20 executions", "host-local journal only", "repeated exact label filters use AND semantics", "--unreconciled is terminal work whose result has never been acknowledged", "terminals that predate acknowledgement tracking are not unreconciled", "paths, prompts, transcripts, and results omitted"}, Examples: [][]string{{"agentctl", "recent"}, {"agentctl", "recent", "--state", "nonterminal", "--label", "review"}, {"agentctl", "recent", "--unreconciled"}}, Related: []string{"status", "await", "result"}},
		"fanout": {
		...
		"await": {
			...
			SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"10 minute timeout", "returns immediately when attention is required", "a terminal return acknowledges the execution so it is no longer unreconciled"},
		...
		"result": {
			...
			SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"stored content or a structured failure is required", "integrity conflicts fail closed", "a successful dereference acknowledges the execution so it is no longer unreconciled"},
			Examples: [][]string{{"agentctl", "result", "exec-..."}, {"agentctl", "result", "exec-...", "--require-result-source", "assistant", "--min-result-bytes", "200"}}, Related: []string{"run", "await", "events", "recent"},
		},
		"subscribe": {
		...
		"route":         {Name: "route", Summary: "Explain a ranked host/model match from a selector string.", Usage: "agentctl route explain [query...]", SideEffectClass: output.ReadOnly, Defaults: []string{"a query string ranks hosts and models from config keywords plus built-in family aliases"}, Examples: [][]string{{"agentctl", "route", "explain", "--", "studio", "omp"}}, Related: []string{"run", "promote"}},
		...
		"skills":        {Name: "skills", Summary: "Plan, inspect, and reconcile a reviewed skill pack from the pinned config-source checkout.", Usage: "agentctl skills plan|status|doctor|reconcile [--manifest path] [--home path] [--harness names]", SideEffectClass: output.LocalOperationalWrite, Defaults: []string{"plan, status, and doctor are read-only and never fetch", "reconcile writes only marker-bound native skill directories", "unmanaged collisions and drift fail closed", "Multica requires a separate advertised runtime-bundle installer", "skills omitted from the manifest are not deleted"}, Examples: [][]string{{"agentctl", "skills", "plan"}, {"agentctl", "skills", "reconcile"}, {"agentctl", "skills", "status"}}, Related: []string{"config source", "bootstrap update", "doctor"}},
